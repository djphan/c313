...
static EVENT_HANDLER(application_ready)
{
    ...
    // Want to turn off sending of messages until an ACK is sent back and read.
...
static EVENT_HANDLER(physical_ready)
{
    ...
    // If router then send without check
    // router can only hold one message (bufer size)
    // Must do all the decision makin 
    if (nodeinfo.nodetype == NT_ROUTER) {
        // Transmit frame needs to be called HERE. It is not since return :/
        int outLink = (link == 1)? nodeinfo.nlinks : 1;
        CHECK(CNET_write_physical(outLink, &f, &len));
        return;
    }

    // This part needs modification  as we set the checksum to 0
    // The f.checksum is set to 0 so that we can caluclate the checksum properly
    checksum    = f.checksum;
    f.checksum  = 0;
    ...
    /* Only for data side or recve side to read.*/
    case DL_DATA :
...
static EVENT_HANDLER(timeouts)
{
    ...
    //
    // Variables exist as globals HERE

    // This is wrong as a time out automatically stops this and resets the identifier
...
EVENT_HANDLER(reboot_node)
{
    //if(nodeinfo.nodenumber > 1) { 
      // Note we will need to generalize this so that a 2+ for lab a2 node network can be processed
    ...
    // Does not enable these actions in the router
    if (nodeinfo.nodetype == NT_HOST) {
        CHECK(CNET_set_handler( EV_APPLICATIONREADY, application_ready, 0));

        // The node with id = 0, is enabled.
        // Therefore in the example: The node that only has the application layer on
        // is Perth (or it's on) to send messages.
        // Sydney only recve messages.
        if(nodeinfo.nodenumber == 0) {
	        CNET_enable_application(ALLNODES);
        }
    }
    CHECK(CNET_set_handler( EV_PHYSICALREADY,    physical_ready, 0));
    // Timer implemented  (req detail for acks recives, expire time, timeouts, etc)
    // cnet defines timeouts and call it once. Require to call function to restart timeut
